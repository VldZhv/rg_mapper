...
- Выгрузка текущей конфигурации на сервер по SSH/SFTP с использованием ключей OpenSSH (`id_rsa`, `*.pem`, `*.key`). Диалог выгрузки заранее заполняет хост (`178.154.195.218`), логин (`radiog`), порт (26015) и целевой каталог (`~/headphones`), чтобы упростить отправку.
