...
## Требования
- Python 3.11+
- Qt-библиотеки, устанавливаемые вместе с `PySide6`.

> **Примечание про PPK-ключи**
> 
> Поддержка PPK-файлов (PuTTY) не входит в обязательные зависимости, так как публичные сборки модулей для работы с PPK недоступны в стандартном репозитории PyPI. Для подключения по SFTP рекомендуется использовать ключи в формате OpenSSH (`id_rsa`, `id_ed25519`, `id_ecdsa`, `*.pem`, `*.key`) либо вручную установить совместимый модуль для PPK или конвертировать ключ через PuTTYgen.

Необходимые зависимости собраны в `requirements.txt`. Установите их командой:
